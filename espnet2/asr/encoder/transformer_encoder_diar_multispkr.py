...
class TransformerEncoder(AbsEncoder):
    ...
    def forward(
        ...
        diar_threshold = torch.where(diar_xs_pad[:] > 0.3, 1, 0)

        spk1_diar = torch.unsqueeze(diar_threshold[..., 0], -1)
        spk2_diar = torch.unsqueeze(diar_threshold[..., 1], -1)

        xs1_pad = xs_pad * spk1_diar
        xs2_pad = xs_pad * spk2_diar
        ...
        if (
            ...
            xs1_pad, masks1 = self.embed(xs1_pad, masks)
            xs2_pad, masks2 = self.embed(xs2_pad, masks)
        else:
            xs1_pad = self.embed(xs1_pad)  
            xs2_pad = self.embed(xs2_pad)
        xs, ms = [None] * self.num_inf, [None] * self.num_inf
        xs[0] = xs1_pad 
        xs[1] = xs2_pad 
        ...
        for ns in range(self.num_inf):
            xs_sd[ns], masks_sd[ns] = self.encoders_sd[ns](xs[ns], ms[ns])
